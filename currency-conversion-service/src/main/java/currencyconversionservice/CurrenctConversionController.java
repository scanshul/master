...
public class CurrenctConversionController {
	...
	@GetMapping(path = "/currency-converter-usingfeign/from/{from}/to/{to}/quantity/{quantity}")
	// http://localhost:8083/currency-converter-usingfeign/from/USD/to/INR/quantity/30
	public CurrencyConverterBean converterCurrencyUsingFeign(@PathVariable String from, @PathVariable String to,
			@PathVariable BigDecimal quantity) {
		CurrencyConverterBean response = currencyExchangeServiceProxy.retriveExchangeValue(from, to);
		return new CurrencyConverterBean(response.getId(), from, to, response.getConversion(), quantity,
				quantity.multiply(response.getConversion()), response.getPort());
	}
